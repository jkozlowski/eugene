 ...
 *
 * Modified to handle BigDecimal values.
 *
...
public class LEAPCodec extends ByteArrayCodec {
    ...
    // Extended primitive type
    private static final byte BIG_DECIMAL = 14;
	...
	private void write(DataOutputStream stream, AbsObject abs) throws Throwable {
        ...
        if (abs instanceof AbsExtendedPrimitive) {
            ...
            Object obj = ((AbsExtendedPrimitive) abs).get();

            if (obj instanceof BigDecimal) {
                writeString(stream, BIG_DECIMAL, ((BigDecimal) obj).toPlainString());
            }
	...
	private synchronized AbsObject read(DataInputStream stream, 
        ...
        if ((type&UNMODIFIER) == BIG_DECIMAL) {
            return AbsExtendedPrimitive.wrap(new BigDecimal(readString(stream, type)));
        }
		if (type == BOOLEAN) {
