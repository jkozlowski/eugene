...
D	& NewOrderSingle & Used by agents wishing to submit securities orders to the Market for execution. &
\texttt{ClOrdID}, \texttt{Symbol}, \texttt{Side}, \texttt{OrderQty}, \texttt{OrdType}, \texttt{Price[Yes]} \\

9	& OrderCancelReject	& Issued by the Market upon receipt of a \texttt{OrderCancelRequest} message which cannot be honored. & 
\texttt{OrderID}, \texttt{ClOrdID}, \texttt{OrdStatus} \\

F	& OrderCancelRequest	& Requests the cancellation of all of the remaining quantity of an existing order. &
